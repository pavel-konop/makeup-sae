...
export default function AdminGalleryPage() {
  ...
  async function handleDelete(publicId: string) {
    ...
    setDeleteError(null);
    const res = await fetch(`/api/admin/delete/${encodeURIComponent(publicId)}`, { method: "DELETE" });
    if (res.ok) {
      setImages((prev) => prev.filter((img) => img.public_id !== publicId));
    } else {
      setDeleteError((await res.json()).error ?? "Deletion failed — please try again.");
    }
    setDeletingId(null);
  }
    ...
    <div className="min-h-screen bg-[#fafafa] flex flex-col">
      ...
      <div className="max-w-5xl mx-auto px-6 py-10 w-full">
        ...
        <div className="bg-white border border-[#eeeeee] p-6 rounded-sm mb-10">
          <h2 className="text-xl text-[#111111] mb-4" style={{ fontFamily: "'Cormorant Garamond', serif", fontWeight: 300 }}>
            Upload new photo
          </h2>
          <form onSubmit={handleUpload} className="flex flex-col sm:flex-row gap-4 items-start">
            ...
            <div className="flex flex-col gap-3 flex-1">
              ...
              <div>
                <label className="block text-xs font-medium text-[#888888] mb-1 tracking-wide uppercase" style={{ fontFamily: "'DM Sans', sans-serif" }}>Photo file</label>
        ...
        {deleteError && (
          <div className="mb-6 px-4 py-3 bg-red-50 border border-red-200 rounded-sm flex items-center justify-between">
            <p className="text-xs text-red-600" style={{ fontFamily: "'DM Sans', sans-serif" }}>{deleteError}</p>
            <button onClick={() => setDeleteError(null)} className="text-xs text-red-400 hover:text-red-600 ml-4">✕</button>
          </div>
        )}

        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl text-[#111111]" style={{ fontFamily: "'Cormorant Garamond', serif", fontWeight: 300 }}>
            Gallery ({images.length} photos)
          </h2>
        </div>

        {images.length === 0
          ? <div className="text-center py-20 text-[#888888] text-sm" style={{ fontFamily: "'DM Sans', sans-serif" }}>No photos yet. Upload the first one above.</div>
          : (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {images.map((img) => (
                <div key={img.public_id} className="relative group aspect-square bg-[#f5f0f2] overflow-hidden rounded-sm">
                  <Image src={img.url} alt={img.public_id.split("/").pop() ?? "Makeup look"} fill className="object-cover" sizes="(max-width: 640px) 50vw, (max-width: 768px) 33vw, 25vw" />
                  <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <button onClick={() => handleDelete(img.public_id)} disabled={deletingId === img.public_id}
